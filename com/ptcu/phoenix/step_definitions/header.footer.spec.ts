...
import {WhatsNew} from '../pages/whats.new.page';
import {browser} from "protractor";
import {SpecHelper} from "../utils/specHelper";

export function headerFooterSpecs() {
    ...
    this.Then(/^I should go to training page$/, () => {
        this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.iotuURL + '/training');
    });
    this.When(/^I browse 'Pricing' from the header$/, () => {
        headerFooter.goToPricing();
    });
    this.Then(/^I should go to Enterprise training page$/, () => {
        this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.iotuURL + '/pricing');
    });
    this.When(/^I browse 'About' from the header$/, () => {
        headerFooter.goToAbout();
    });
    this.Then(/^I should go to About us page of PTCU$/, () => {
        this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.iotuURL + '/about-us');
    });
    ...
    this.Then(/^I should go to Blog application of PTCU$/, () => {
        headerFooter.runOnNonAngular(() => {
            browser.sleep(10000);
            this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.blogURL);
        }).catch((error: any) => {
    ...
    this.Then(/^I should go to catalog page$/, () => {
        this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.iotuURL + '/catalog');
    });
    this.When(/^I browse 'Tutorials' from the footer$/, () => {
        headerFooter.goToTutorialsFromFooter();
    });
    this.Then(/^I should go to tutorials page$/, () => {
        this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.iotuURL + '/video-library');
    });
    ...
    this.Then(/^I should go to PLMS page$/, () => {
        ...
        headerFooter.runOnNonAngular(() => {
            //  browser.sleep(10000);
            this.expect(browser.driver.getCurrentUrl()).to.be.eventually.contains(browser.params.URLOptions.enterpriseURL);
        });
    ...
    this.Then(/^I should go to Find a Training Page on the support site of PTC$/, () => {
        ...
        this.expect(browser.getCurrentUrl()).to.be.eventually.contains(browser.params.URLOptions.supportURL + '/apps/ext/training/search.jsp?product=');
    });
    ...
    this.Then(/^I should go to Talk to a Training Advisor page of PTC site$/, () => {
        ...
        headerFooter.runOnNonAngular(() => {
            browser.sleep(10000);
            this.expect(browser.driver.getCurrentUrl()).to.be.eventually.contains(browser.params.URLOptions.PTCURL + '/en/education');
        });
        /*headerFooter.getBrowserErrors((logs:any)=>{
            console.log('Log is Severe and message is'+logs);
            this.expect(logs.length).to.equal(0);
    ...
    this.Then(/^I should go to specialization page of PTCU$/, () => {
        this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.iotuURL + '/enrollment/specialization/thingworx-iot-developer-specialization');
        /*  headerFooter.getBrowserErrors((logs:any)=>{
              console.log('Log is Severe and message is'+logs);
              this.expect(logs.length).to.equal(0);
          });*/
    ...
    this.Then(/^I should go to learning services page of PTC site$/, () => {
        ...
        headerFooter.runOnNonAngular(() => {
            ...
            this.expect(headerFooter.getURL()).to.be.eventually.equals(browser.params.URLOptions.PTCURL + '/en/education/learning-services');
        });
        headerFooter.getBrowserErrors((logs: any) => {
            console.log('Log is Severe and message is' + logs);
            this.expect(logs.length).to.equal(0);
    ...
    this.Then(/^I should go to forum homepage$/, () => {
        ...
        headerFooter.runOnNonAngular(() => {
            ...
            this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.forumURL);
    ...
    this.Then(/^I should go to overview section of help page$/, () => {
        this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.iotuURL + '/help');
    });
    ...
    this.Then(/^I should go to resources page$/, () => {
        ...
        this.expect(browser.getCurrentUrl()).to.be.eventually.equals(browser.params.URLOptions.iotuURL + '/resources');
        headerFooter.getBrowserErrors((logs: any) => {
            console.log('Log is Severe and message is' + logs);
            this.expect(logs.length).to.equal(0);
        });
    ...
    this.Then(/^I should go to the contact us section of help page$/, () => {
        this.expect(headerFooter.getURL()).to.eventually.be.equals(browser.params.URLOptions.iotuURL + '/contact-us');
    });
    ...
    this.Then(/^I should go to a privacy policy page of PTC site$/, () => {
        ...
        headerFooter.runOnNonAngular(() => {
            //   browser.sleep(10000);
            this.expect(browser.getCurrentUrl()).to.eventually.be.equals(browser.params.URLOptions.PTCURL + '/en/documents/policies/privacy');
        });
    ...
    this.Then(/^I should go to Terms of Use section of help page$/, () => {
        ...
        var PTC_WINDOW = 1;
        specHelper.switchWindow(PTC_WINDOW);
        this.expect(headerFooter.getURL()).to.be.eventually.equals(browser.params.URLOptions.iotuURL + '/terms-of-use');
