import {trainingLandingPage} from "../pages/training.landing.page";
import {browser, by} from "protractor";
import {ok} from "assert";

const chai = require("chai").use(require("chai-smoothie"));
const expect = chai.expect;
let domainURL = browser.params.URLOptions.iotuURL;
let trainingFeatures = ['Learn On Demand',
    'Upskill in Emerging Technologies',
    'Connect With The Community'];

export function trainingLandingSpec() {
    let trainingLandingPagePO = new trainingLandingPage();

    this.Then(/^I should see the "([^"]*)" page heading$/, (pageHeading: string) => {
        expect(trainingLandingPagePO.getTrainingPageHeading()).to.be.eventually.equals(pageHeading);
    });
    this.Then(/^I should see the "([^"]*)" on description$/, (pageDescription: string) => {
        expect(trainingLandingPagePO.getTrainingPageDescription()).to.be.eventually.equals(pageDescription);
    });
    this.Then(/^I should see the page title as "([^"]*)"$/, function (pageTitle: string) {
    ...
    this.Then(/^I should see training features and its details$/, function () {
        ...
        trainingLandingPagePO.getTrainingFeatures().each(function (element, index) {
            element.element(by.tagName('h3')).getText().then(function (text) {
                console.log(text);
